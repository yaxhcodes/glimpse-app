...
class PlatformIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (builder == null) return Icon(Icons.circle_outlined, size: size, color: c);
    return CustomPaint(
...
void _reddit(Path p, double s) {
  ...
  p.addOval(Rect.fromCircle(center: Offset(cx - s * 0.22, cy - s * 0.06), radius: s * 0.045));
  p.addOval(Rect.fromCircle(center: Offset(cx + s * 0.22, cy - s * 0.06), radius: s * 0.045));
  p.moveTo(cx, cy - r);
  p.lineTo(cx + s * 0.14, cy - r - s * 0.18);
  p.addOval(Rect.fromCircle(center: Offset(cx + s * 0.14, cy - r - s * 0.18), radius: s * 0.04));
}
...
void _instagram(Path p, double s) {
  ...
  p.addOval(Rect.fromCircle(center: Offset(cx + s * 0.24, cy - s * 0.24), radius: s * 0.035));
}
